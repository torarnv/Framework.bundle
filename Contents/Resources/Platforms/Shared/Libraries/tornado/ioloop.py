    ...
    def start(self):
        ...
        while True:
            ...
            try:
                ...
                if ((hasattr(e, 'errno') and getattr(e, 'errno') in (errno.EINTR, errno.EBADF)) or
                    (isinstance(getattr(e, 'args'), tuple) and len(e.args) == 2 and e.args[0] in (errno.EINTR, errno.EBADF))):
...
class _Select(object):
    ...
    def poll(self, timeout):
        ...
        try:
          ...
          # Sometimes we get a bad file descriptor. This can cause an infinite loop for some reason
          # when the FD doesn't get removed from the set (not sure why), and then select keeps failing
          # over and over again).
          #
          if ((hasattr(e, 'errno') and getattr(e, 'errno') == errno.EBADF) or
              (isinstance(getattr(e, 'args'), tuple) and len(e.args) == 2 and e.args[0] == errno.EBADF)):
            
            # Get a union of all the FDs.
            fds = {}
            for fd in self.read_fds | self.write_fds | self.error_fds:
              fds[fd] = True
              
            events = {}
            for fd in fds.keys():
              try:
                # See if this FD is valid, but don't wait.
                select.select([fd], [], [], 0)
              except:
                # Remove the file descriptor from all the sets, just in case. If we don't do
                # this an infinite loop has been observed where select() keeps failing.
                #
                try: self.read_fds.remove(fd)
                except: pass
                try: self.write_fds.remove(fd)
                except: pass
                try: self.error_fds.remove(fd)
                except: pass
                events[fd] = events.get(fd, 0) | IOLoop.ERROR
