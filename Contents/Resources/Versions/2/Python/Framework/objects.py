...
class XMLObject(Object):
  ...
  def ToElement(self, context=None):
    ...
    for key in self.__dict__:
      if key[0] != "_" and self.__dict__[key].__class__.__name__ != "function" and key != "tagName" and self.__dict__[key] != None:
        ...
        if key == "ratingKey":
          el.set(key, urllib.quote(base64.urlsafe_b64encode(self.__dict__[key])))
        elif value == True:
          el.set(key, "1")
        elif value == False:
          el.set(key, "0")
          ...
          value = self.__dict__[key]
          if isinstance(value, (Framework.components.localization.LocalString, Framework.components.localization.LocalStringPair)) and context and Framework.constants.header.language in context.headers:
            el.set(key, unicode(value.localize(context.headers[Framework.constants.header.language])))
          else:
            el.set(key, unicode(value))
    return el
        
  def Content(self, context=None):
    return self._core.data.xml.to_string(self.ToElement(context=context))
...
class XMLContainer(XMLObject, Container):
  ...
  def ToElement(self, context=None):
    root = XMLObject.ToElement(self, context=context)
    for item in self.__items__:
      if isinstance(item, XMLObject):
        el = item.ToElement(context=context)
        if el != None:
          root.append(el)
    return root
...
class ContextMenu(XMLContainer):
  ...
  def ToElement(self, sender, contextKey, context=None, **kwargs):
    root = XMLObject.ToElement(self, context=context)
    for item in self.__items__:
  ...
  def ToElement(self, context=None):
    ...
    for item in self.__items__:
      ...
      if isinstance(item, XMLObject):
        ...
        __itemElement = item.ToElement(context=context)
        ...
        if __itemContextKey:
          ...
          if __contextMenu:
            __itemElement.append(__contextMenu.ToElement(info, __itemContextKey, context=context, **__itemContextArgs))
...
class MessageContainer(XMLContainer):
  def __init__(self, core, header, message, title1=None, title2=None, **kwargs):
    XMLContainer.__init__(self, core, header=header, message=message, title1=title1, title2=title2, **kwargs)
...
class WebVideoItem(XMLObject):
  def __init__(self, core, url, title=None, subtitle=None, summary=None, duration=None, thumb=None, art=None, **kwargs):
    prefix = core.runtime.prefix_handlers.keys()[0]
...
class Function(XMLObject):
  ...
  def ToElement(self, context=None):
    ...
    self.key = "%s/:/function/%s%s%s" % (prefix, self.__obj.key.__name__, self.__ext, queryString)
    return XMLObject.ToElement(self, context=context)
  ...
  def __str__(self):
    if hasattr(self, 'key'):
      if type(self.key) == types.FunctionType:
        ...
        s = "%s/:/function/%s%s%s" % (self._core.runtime.prefix_handlers.keys()[0], self.key.__name__, self.__ext, queryString)
        return s
...
class PrefsItem(DirectoryItem):
  def __init__(self, core, title=None, subtitle=None, summary=None, thumb=None, **kwargs):
    DirectoryItem.__init__(self, core, "%s/:/prefs" % core.runtime.prefix_handlers.keys()[0], title=title, subtitle=subtitle, summary=summary, thumb=thumb, settings=True, **kwargs)
...
cerealizer.register(IndirectFunction)
cerealizer.register(PrefsItem)
cerealizer.register(Redirect)
cerealizer.register(MetadataSearchResult)