...
class ExternalFunction(Framework.bases.Base):
...
class FunctionBroker(Framework.bases.Base):
...
class ExternalPluginBroker(Framework.bases.Base):
  ...
  def Wait(self, identifier, timeout=None):
    if timeout == 0:
      timeout = None
    if ':' in identifier:
      raise Framework.FrameworkException("The ':' character cannot be used in plug-in identifiers.")
...
class Notification(Framework.bases.Base):
...
class NotificationBroker(Framework.bases.Base):
  def __getattr__(self, name):
    return Notification(self._core, name)

##################################################################################################

class Trigger(Framework.bases.Base):
  def __init__(self, broker, name):
    Framework.bases.Base.__init__(self, broker._core)
    self._broker = broker
    self._name = name
...
class TriggerBroker(Framework.bases.Base):
...
class TriggerManager(Framework.bases.Base):
  def _init(self):
...
class Source(Framework.bases.Base):
  def __init__(self, core, identifier):
    Framework.bases.Base.__init__(self, core)
    self._identifier = identifier
...
class SourceBroker(Framework.bases.Base):
...
class MessageKit(Framework.bases.BaseKit):
  """
  ...
  def _init(self):
    self._globals = dict(
      expose    = self._expose_decorator,
      External  = ExternalPluginBroker(self._core),
      event     = EventBroker(),
      source    = SourceBroker(self._core),
      Notify    = NotificationBroker(self._core),
      trigger   = TriggerManager(self._core)
    )
