...
class Serializable(object):
  ...
  def _serialize(self, path):
    raise Framework.FrameworkException("_serialize not implemented in subclass %s" % type(self).__name__)
    
  def _deserialize(self, path, el):
    raise Framework.FrameworkException("_deserialize not implemented in subclass %s" % type(self).__name__)
