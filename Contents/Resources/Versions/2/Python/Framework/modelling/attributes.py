...
class AttributeSet(object):
  ...
  def __setattr__(self, name, value):
    ...
    # Only allow setting the id if the model isn't read-only
      ...
      if self._access_point._read_only:
        raise Framework.FrameworkException("Data for %s instance %s provided by %s can't be modified" %  (type(self).__name__, self._uid, self.provider))
      self._id = value
  ...
  def _get_synthetic_attr(self, name):
    ...
    raise Framework.FrameworkException("No synthetic attribute named '%s' found" % name)
...
class AttributeObject(object):
  ...
  def _setcontent(self, content):
    if self._access_point._read_only:
      raise Framework.FrameworkException("Data for %s instance %s provided by %s can't be modified" %  (type(self._model).__name__, self._model._uid, self._access_point._identifier))
...
class ValueObject(AttributeObject, Serializable):
  ...
  def _setcontent(self, content):
    ...
    if type(content) != type(self)._type and content != None:
      raise Framework.FrameworkException("Provided content of type %s is not of the required type %s." % (str(type(content)), str(type(self)._type)))
    self._value = content
    self._save()
...
class StringObject(ValueObject):
  ...
  def _setcontent(self, content):
    ...
    ValueObject._setcontent(self, unicode(content))
...
class DataObject(AttributeObject, Serializable):
  ...
  def _serialize(self, path):
    if not self._external:
      raise Framework.FrameworkException("Data objects must always be saved externally.")
    else:
...
class ContainerObject(AttributeObject, Serializable):
  ...
  def _setcontent(self, content):
    raise Framework.FrameworkException("Container-type attributes can't be assigned directly.")
...
class MapItem(ItemObject, Serializable):
  ...
  def _serialize(self, path):
    if not isinstance(self._obj, Serializable):
      raise Framework.FrameworkException("Mapping item %s is not serializable!" % repr(self._obj))
...
class DirectoryObject(MapObject, Serializable, Combinable):
  ...
  def _init(self):
    if not isinstance(self._template._item_template, templates.DataTemplate):
      raise Framework.FrameworkException("Incorrect template type (%s): Directory attributes can only hold Data items." % self._template._item_template)
...
class ProxyObject(object):
  ...
  def __init__(self, proxy_name, proxy_type, data, sort_order=None, ext=None):
...
class ProxyContainerItem(MapItem):
  ...
  def __init__(self, obj, name):
    if not isinstance(obj, ProxiedDataObject):
      raise Framework.FrameworkException("Incorrect object type (%s): Must be a Proxied Data object." % type(obj))
...
class ProxyContainerObject(MapObject, Serializable, Combinable):
  ...
  def _init(self):
    if not isinstance(self._template._item_template, templates.DataTemplate):
      raise Framework.FrameworkException("Incorrect template type (%s): ProxyContainer attributes can only hold Data items." % self._template._item_template)
    MapObject._init(self)
  ...
  def __setitem__(self, name, value):
    if not isinstance(value, ProxyObject):
      raise Framework.FrameworkException("Incorrect value type (%s): Must be a Proxy object." % type(value))
    if value._proxy_name not in self._template._accepted_proxies:
      raise Framework.FrameworkException("Proxy type '%s' is not accepted by this attribute." % value._proxy_name)
    MapObject.__setitem__(self, name, value)
      
class SetItem(ItemObject, Serializable):
  def _serialize(self, path):
    if not isinstance(self._obj, Serializable):
      raise Framework.FrameworkException("Set item %s is not serializable!" % repr(self._obj))
    attr_el = self._obj._serialize(path)
    return attr_el
...
class SetObject(ContainerObject, Serializable, Combinable):
  ...
  def new(self):
    if not isinstance(self._template._item_template, templates.RecordTemplate):
      raise Framework.FrameworkException("This attribute type does not support creating new instances.")
    obj = self._template._item_template._new(self, 'item')
    self._items.append(obj)
    self._save()
    return obj
...
class LinkObject(AttributeObject, Serializable):
  ...
  def _setcontent(self, content):
    ...
    if str(type(content)) != str(cls):
      raise Framework.FrameworkException("Attribute must be set to a %s object" % cls.__name__)
    else:
