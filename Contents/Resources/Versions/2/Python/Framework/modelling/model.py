  ...
  def make_guid(cls, id, **kwargs):
    for key in kwargs:
      if key not in cls._template.guid_args:
        raise Framework.FrameworkException("Invalid GUID argument '%s' for model %s" % (key, cls.__name__))
    for key in cls._template.guid_args:
      if key not in kwargs:
        raise Framework.FrameworkException("Missing GUID argument '%s' for model %s" % (key, cls.__name__))
...
class Model(AttributeSet, Serializable):
  ...
  def _write(self, subdir=None):
    ...
    try:
      ...
      self._core.log_except(None, "Exception serializing %s with guid '%s'", type(self).__name__, self._uid)
    finally:
